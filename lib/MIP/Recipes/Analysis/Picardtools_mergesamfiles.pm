...
## Function : Merges all bam files using Picardtools mergesamfiles within each sampleid and files generated previously
## (option if provided with '-picardtools_mergesamfiles_previous_bams'). The merged files have to be sorted before attempting to merge.
## Returns  : |$xargs_file_counter
## Arguments: $active_parameter_href   => Active parameters for this analysis hash {REF}
##          : $family_id               => Family id
##          : $file_info_href          => File_info hash {REF}
...
##          : $xargs_file_counter      => The xargs file counter
    ...
    ## Note: Merging is done through sample_id and directories are created based on sample_id.
    ## TODO: Create directories based on sample_origin
    ## TODO: Copy files based on sample_origin
    ...
    ( $file_path, $program_info_path ) = setup_script(
        {
            active_parameter_href           => $active_parameter_href,
            core_number                     => $core_number,
            directory_id                    => $sample_id, ## TODO: does this need to be changed to family id?
            FILEHANDLE                      => $FILEHANDLE,
            job_id_href                     => $job_id_href,
            process_time                    => $time,
            program_directory               => $outaligner_dir,
    ...
    my $infile_tag = $file_info_href->{$sample_id}
      { $parameter_href->{active_aligner} }{file_tag};
    my $outfile_tag =
      $file_info_href->{$sample_id}{ppicardtools_mergesamfiles}{file_tag}; ## TODO: file tag is fine, double check parameters
    ...
    ## Set helper value for finding merged_infiles downstream
    my $merged_infile_prefix =
      $sample_id . $UNDERSCORE . q{lanes} . $UNDERSCORE . $lanes_id; ## TODO: Prefix based on family id + normal/tumor
    set_merged_infile_prefix(
        {
            file_info_href       => $file_info_href,
            merged_infile_prefix => $merged_infile_prefix,
  ...
  INFILE:
    ...
    ## More than one file - we have something to merge
    ## TODO: Still merge and create based on sample origin
    if ( scalar @{ $infile_lane_prefix_href->{$sample_id} } > 1 ) {
...
q{## Renaming sample instead of merge to streamline handling of filenames downstream};
      ...
      CONTIG:
        foreach my $contig ( @{ $file_info_href->{contigs_size_ordered} } ) {
            ## TODO: Again, sample id sbould be altered to family id + sample origin
    ...
    ## Copies file from temporary directory. Per contig
    ## TODO: Again, sample id here. It should update based on sample_origin
    say {$FILEHANDLE} q{## Copy file from temporary directory};
    ($xargs_file_counter) = xargs_migrate_contig_files(
        {
            contigs_ref  => \@{ $file_info_href->{contigs_size_ordered} },
